...
<!-- 			<form method="post" action="../qna/admin_update_ok.do"> -->
			<form method="post" action="../qna/admin_update.do">
						...
						<td id="rmsg" style="text-align: left;">
 						<td width="80%">
							<select name="type" class="notice-form-select">
								<option value="1" ${vo.cate==1?"selected":"" }>전체</option>
								<option value="2" ${vo.cate==2?"selected":"" }>웨딩홀</option>
								<option value="3" ${vo.cate==3?"selected":"" }>드레스/예복</option>
								<option value="4" ${vo.cate==4?"selected":"" }>스튜디오/헤메</option>
								<option value="5" ${vo.cate==5?"selected":"" }>혼수</option>
							</select>
                        </td>
					</tr>
					<tr id=white>
						<th width="20%">작성자</th>
						<td width="80%">
							<input type="text" name="subject" class="notice-form-input" size="50" required value="${vo.id}">
							<input type="hidden" name="no" value="${vo.qna_no }">
						</td>
					</tr>
					<tr>
						<th width="20%">내용</th>
						<td width="80%">
							<textarea rows="10" cols="50" name="content" class="notice-form-textarea" required>${vo.msg}</textarea>
						</td>
					</tr>
					<tr id="noBtn">
						<td colspan="2">
